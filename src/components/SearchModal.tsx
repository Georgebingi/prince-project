      ...
      <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center border-b border-slate-200 px-4 py-3">
          ...
          <input
            ref={inputRef}
            type="text"
            placeholder="Search cases, documents, staff..."
            className="flex-1 bg-transparent border-none text-lg focus:outline-none placeholder:text-slate-400"
            value={query}
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          ...
          <div className="p-8 text-center text-slate-500">
              ...
              <div className="mt-4 flex flex-wrap justify-center gap-2">
                <span
                className="px-2 py-1 bg-slate-100 rounded text-xs cursor-pointer hover:bg-slate-200"
                onClick={() => setQuery('State vs')}>

                  State vs...
                </span>
                <span
                className="px-2 py-1 bg-slate-100 rounded text-xs cursor-pointer hover:bg-slate-200"
                onClick={() => setQuery('Affidavit')}>

                  Affidavit
                </span>
                <span
                className="px-2 py-1 bg-slate-100 rounded text-xs cursor-pointer hover:bg-slate-200"
                onClick={() => setQuery('Justice')}>
          ...
          <div className="space-y-1">
            ...
            <div
              ...
              className="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-lg cursor-pointer group transition-colors">

                  <div
                className={`p-2 rounded-lg ${result.type === 'case' ? 'bg-blue-100 text-blue-600' : result.type === 'document' ? 'bg-amber-100 text-amber-600' : 'bg-emerald-100 text-emerald-600'}`}>

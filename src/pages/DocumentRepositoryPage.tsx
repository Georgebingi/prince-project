...
export default function DocumentRepositoryPage() {
  ...
  const [selectedDoc, setSelectedDoc] = useState<CaseDocument & { caseId: string; caseTitle: string } | null>(null);
  ...
  const allDocuments = useMemo(() => {
    ...
    cases.forEach((c) =>
      c.documents.forEach((doc) => docs.push({ ...doc, caseId: c.id, caseTitle: c.title }))
    );
    return docs.sort(
  ...
  const handleDownload = async (doc: CaseDocument & { caseId: string; caseTitle: string }) => {
    try {
    ...
    <Layout title="Document Repository" showLogoBanner={false}>
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Select
                ...
                options={[{ value: '', label: 'Choose a case...' }, ...cases.map((c) => ({ value: c.id, label: `${c.id} - ${c.title}` }))]}
                required
              />
              <Select
                label="Document Category"
                value={selectedCategory}
                onChange={(e) => setSelectedCategory(e.target.value as 'judgment' | 'affidavit' | 'evidence' | 'admin')}
                options={[
              ...
              {uploading ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    {selectedCaseId && (
                      <Button size="sm" className="mt-2" onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }}>Select Files</Button>
                    )}
                  </div>
        ...
        <div className="flex flex-col md:flex-row justify-between gap-4 items-center">
          <h3 className="text-lg font-semibold text-slate-900 self-start md:self-center">My Files</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {folders.map((folder) => (
            <button key={folder.name} onClick={() => setFilter(folder.type as 'all' | 'judgment' | 'evidence' | 'affidavit' | 'admin')} className={`flex flex-col items-center p-4 bg-white border rounded-lg hover:shadow-md transition-all group text-center ${filter === folder.type ? 'border-primary ring-1 ring-primary bg-blue-50' : 'border-slate-200 hover:border-blue-300'}`}>
              ...
              <span className={`font-medium text-sm ${filter === folder.type ? 'text-primary' : 'text-slate-700'}`}>{folder.name}</span>
        ...
        <Card noPadding>
          <div className="p-4 border-b border-slate-100 flex justify-between items-center">
            <span className="font-semibold text-slate-900">{filter === 'all' ? 'All Documents' : `${filter.charAt(0).toUpperCase() + filter.slice(1)} Files`}</span>
              ...
              filteredDocs.map((doc) => (
                <div key={doc.id} className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors group cursor-pointer" onClick={() => setSelectedDoc(doc)}>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3 text-xs text-slate-500 mt-1">
                        <Badge variant="secondary" className="text-[10px] py-0 h-5">{doc.category || 'uncategorized'}</Badge>
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <Button size="sm" variant="ghost" title="Download" onClick={(e) => { e.stopPropagation(); handleDownload(doc); }}>
          ...
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-8 bg-slate-50 min-h-[400px] flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-slate-500 mb-2"><strong>Document:</strong> {selectedDoc.name}</p>
                <p className="text-sm text-slate-400 mb-2"><strong>Case:</strong> {selectedDoc.caseId} - {selectedDoc.caseTitle}</p>
                <p className="text-sm text-slate-400 mb-2"><strong>Category:</strong> {selectedDoc.category}</p>
                <p className="text-sm text-slate-400 mb-2"><strong>Type:</strong> {selectedDoc.type}</p>
                <p className="text-sm text-slate-400 mb-4"><strong>Size:</strong> {selectedDoc.size}</p>
